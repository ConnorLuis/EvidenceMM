...
from evidencemm.motion_selection import MotionAwareSelection
...
class MotionCoverageResult(BaseModel):
...
def evaluate_motion_event_coverage(
    *,
    annotations: list[TemporalEventAnnotation],
    records: list[FrameRecord],
    selections: list[MotionAwareSelection],
) -> MotionCoverageResult:
    verified = [
        item
    ...
    if not selections:
        raise ValueError("no motion-aware selections")
    ...
    if len(set(slice_ids)) != len(slice_ids):
        raise ValueError("duplicate motion selection slice_group_id")
    ...
    selected_rows = [
        {
            ...
            "front_motion_score": item.front_motion_score,
            "wrist_motion_score": item.wrist_motion_score,
            "fused_motion_score": item.fused_motion_score,
        }
        for item in selections
    ]

    return MotionCoverageResult(
        verified_events=len(verified),
        covered_events=covered_count,
        event_coverage=covered_count / len(verified),
        shared_sample_budget=len(selected_indices),
        evidence_image_budget=len(selected_indices) * 2,
        events=rows,
