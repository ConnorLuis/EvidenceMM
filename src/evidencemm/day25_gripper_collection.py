...
GRIPPER_DIRECTION = "late"
GRIPPER_MIN_MM = 30.0
GRIPPER_MAX_MM = 40.0
GRIPPER_UNIT = "mm"
GRIPPER_OPERATIONAL_MEASUREMENT = "upward_progress_after_nominal_close_point_before_close"

PLAN_FIELDS = (
    "schema_version", "day25_sequence", "plan_row_id", "pair_group_id",
...
RECORD_FIELDS = (
    ...
    "changed_factor_observable", "phase_proxy_met", "parameter_direction",
...
def expected_day25_rows_from_day22(day22_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for pair_index in range(1, 16):
        ...
        primary = [
            ...
            if row["slot_index"] == "3"
            and row["planned_physical_cause"] == "gripper_close_timing"
        ]
        repeat = [
            row for row in members
            if row["slot_index"] == "5"
        ...
        if len(primary) != 1:
            raise ValueError(f"Day22 group {group} missing gripper-primary slot")
        for src in primary + repeat:
            selected.append({
                "schema_version": "evidencemm_day25_gripper_collection_plan_v1",
                "day25_sequence": str(sequence),
                ...
                "parameter_direction": GRIPPER_DIRECTION,
                "parameter_min": f"{GRIPPER_MIN_MM:g}",
                "parameter_max": f"{GRIPPER_MAX_MM:g}",
                "parameter_unit": GRIPPER_UNIT,
                "operational_measurement": GRIPPER_OPERATIONAL_MEASUREMENT,
...
def validate_day25_plan_shape(rows: list[dict[str, str]]) -> None:
    if len(rows) != 20:
        raise ValueError(f"Day25 plan must contain 20 rows, got {len(rows)}")
    if {row["pair_group_id"] for row in rows} != set(EXPECTED_PAIR_GROUPS):
        raise ValueError("Day25 plan pair-group set mismatch")

    repeat = [row for row in rows if parse_bool(row["repeat_slot"])]
    if len(repeat) != 5:
        raise ValueError(f"expected 5 gripper repeats, got {len(repeat)}")
    if {row["pair_group_id"] for row in repeat} != set(GRIPPER_REPEAT_GROUPS):
        raise ValueError("gripper repeat groups mismatch")

    for group in EXPECTED_PAIR_GROUPS:
        group_rows = [row for row in rows if row["pair_group_id"] == group]
        expected = 2 if group in GRIPPER_REPEAT_GROUPS else 1
        if len(group_rows) != expected:
            raise ValueError(f"{group}: expected {expected} gripper rows")

    for row in rows:
        if row["planned_physical_cause"] != "gripper_close_timing":
            raise ValueError(f"{row['plan_row_id']}: unexpected cause")
        if row["planned_intervention_type"] != "manual_gripper_close_timing_shift":
            raise ValueError(f"{row['plan_row_id']}: intervention type drift")
        if row["parameter_direction"] != GRIPPER_DIRECTION:
            raise ValueError(f"{row['plan_row_id']}: direction drift")
        if float(row["parameter_min"]) != GRIPPER_MIN_MM:
            raise ValueError(f"{row['plan_row_id']}: parameter_min drift")
        if float(row["parameter_max"]) != GRIPPER_MAX_MM:
            raise ValueError(f"{row['plan_row_id']}: parameter_max drift")
        if row["parameter_unit"] != GRIPPER_UNIT:
            raise ValueError(f"{row['plan_row_id']}: unit drift")
        if row["operational_measurement"] != GRIPPER_OPERATIONAL_MEASUREMENT:
...
def evaluate_experimental_validity(
    *, plan_row: dict[str, str], technical_valid: bool, task_success: bool,
    ...
    changed_factor_observable: bool, phase_proxy_met: bool,
    ...
    if plan_row["planned_physical_cause"] != "gripper_close_timing":
        return False, "unexpected_day25_cause"
    if not intervention_applied:
        return False, "declared_gripper_intervention_not_applied"
    ...
    if not phase_proxy_met:
        return False, "late_close_phase_proxy_not_met"
    if task_success:
        return False, "gripper_intervention_did_not_induce_failure"
    return True, ""
...
def build_record(
    *, plan_row: dict[str, str], audit: TechnicalAudit, attempt_index: int,
    ...
    phase_proxy_met: bool, safety_abort: bool, hardware_fault: bool,
    ...
    experimental_valid, exclusion_reason = evaluate_experimental_validity(
        ...
        phase_proxy_met=phase_proxy_met, safety_abort=safety_abort,
        hardware_fault=hardware_fault,
    )
    return {
        "schema_version": "evidencemm_day25_gripper_collection_record_v1",
        ...
        "phase_proxy_met": bool_text(phase_proxy_met),
        "parameter_direction": GRIPPER_DIRECTION,
        "parameter_min": f"{GRIPPER_MIN_MM:g}",
        "parameter_max": f"{GRIPPER_MAX_MM:g}",
        "parameter_unit": GRIPPER_UNIT,
...
def analyze_collection(
    *, plan_rows: list[dict[str, str]], records: list[dict[str, str]],
    ...
    validate_day25_plan_shape(plan_rows)
    ...
    for group in EXPECTED_PAIR_GROUPS:
        ...
        per_group[group] = {
            ...
            "expected_gripper_slots": len(expected_rows),
            "canonical_gripper_slots": len(selected_rows),
    ...
    return {
        "schema_version": "evidencemm_day25_gripper_collection_analysis_v1",
        "status": "complete" if len(canonical) == 20 else "in_progress",
        "new_attempt_count": len(records),
        "new_gripper_canonical_count": len(canonical),
        "expected_new_gripper_canonical_count": 20,
        "gripper_failure_count": sum(not parse_bool(r["task_success"]) for r in canonical),
        ...
        "gripper_parameter": {
            ...
            "operational_measurement": GRIPPER_OPERATIONAL_MEASUREMENT,
            "magnitude_range": [GRIPPER_MIN_MM, GRIPPER_MAX_MM],
            "unit": GRIPPER_UNIT,
...
def validate_final_analysis(analysis: dict[str, Any]) -> list[str]:
    errors = []
    expected = {
        "new_gripper_canonical_count": 20,
        "gripper_failure_count": 20,
    ...
    if analysis.get("new_clean_collection_required") is not False:
        errors.append("Day25 must reuse Day24 clean anchors")
    if analysis.get("future_split_materialized") is not False:
        errors.append("future split must not be materialized on Day25")

    p = analysis.get("gripper_parameter") or {}
    if p.get("direction") != GRIPPER_DIRECTION:
        errors.append("gripper direction mismatch")
    if p.get("operational_measurement") != GRIPPER_OPERATIONAL_MEASUREMENT:
        errors.append("gripper measurement mismatch")
    if list(p.get("magnitude_range") or []) != [GRIPPER_MIN_MM, GRIPPER_MAX_MM]:
        errors.append("gripper magnitude range mismatch")
    if p.get("unit") != GRIPPER_UNIT:
        errors.append("gripper unit mismatch")
