...
from evidencemm.day26_trajectory_collection import (
    analyze_collection,expected_day26_rows_from_day22,selected_day24_clean_anchors,
    validate_day26_plan_shape,validate_final_analysis)

def args():
    p=argparse.ArgumentParser()
    p.add_argument("--day22-plan",type=Path,default=Path("data/protocol/day22_root_cause_collection_plan.csv"))
    p.add_argument("--day26-plan",type=Path,default=Path("data/protocol/day26_trajectory_collection_plan.csv"))
...
def main():
    ...
    actual=load_csv(a.day26_plan)
    expected=expected_day26_rows_from_day22(day22)
    validate_day26_plan_shape(actual)
    if comparable(actual)!=comparable(expected):
        raise SystemExit("DAY26 VALIDATION FAIL: derived Day26 plan does not match frozen Day22 plan")
    ...
    if errors:
        print("===== DAY26 TRAJECTORY COLLECTION: NOT READY =====")
        for e in errors: print("FAIL:",e)
        print(f"CURRENT: trajectory={analysis['new_trajectory_canonical_count']}/20 "
              f"failures={analysis['trajectory_failure_count']}/20 "
              f"clean_anchors={analysis['clean_anchor_count']}/15 "
              f"groups={analysis['complete_pair_group_count']}/15")
        return 2
    print("===== DAY26 TRAJECTORY COLLECTION: PASS =====")
    print("frozen_day22_plan_sha256: PASS")
    print("derived_trajectory_plan: 20/20 PASS")
    print("day24_clean_anchors: 15/15 PASS")
    print("pair_groups: 15/15 PASS")
    print("trajectory_failure: 20/20 PASS")
    print("trajectory_parameter: follower_forward 40-60 mm marked lateral waypoint offset PASS")
