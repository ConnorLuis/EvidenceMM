...
from evidencemm.day25_gripper_collection import analyze_collection,selected_day24_clean_anchors

def args():
    p=argparse.ArgumentParser()
    p.add_argument("--plan",type=Path,default=Path("data/protocol/day25_gripper_collection_plan.csv"))
    ...
    p.add_argument("--output-json",type=Path,default=Path("data/protocol/day25_gripper_collection_analysis.json"))
    p.add_argument("--output-csv",type=Path,default=Path("data/protocol/day25_gripper_collection_analysis.csv"))
    return p.parse_args()
...
def main():
    ...
    with a.output_csv.open("w",newline="",encoding="utf-8") as h:
        w=csv.DictWriter(h,fieldnames=fields); w.writeheader(); w.writerows(analysis["canonical_details"])
    print("===== DAY25 GRIPPER COLLECTION ANALYSIS =====")
    print(f"gripper_canonical={analysis['new_gripper_canonical_count']}/20 "
          f"gripper_failure={analysis['gripper_failure_count']}/20 "
