...
def main() -> int:
    ...
    result = {
        ...
        "index_build_stats": index["stats"],
        "cases": rows,
        "scope_note": (
            "Day 4 smoke baseline over one 8-page "
            "datasheet and two verified PDF queries; "
            "not a headline benchmark."
        ),
    }

    day3 = ROOT / "reports/day3_text_retrieval.json"
    if day3.exists():
        previous = json.loads(
            day3.read_text(encoding="utf-8")
        )
        result["day3_text_only_reference"] = {
            key: previous[key]
            for key in [
                "recall_at_1",
                "recall_at_3",
