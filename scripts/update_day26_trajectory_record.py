...
from evidencemm.day24_target_collection import (
    audit_episode, load_csv, next_attempt_index, parse_bool, selected_for_plan,
)
from evidencemm.day26_trajectory_collection import RECORD_FIELDS, build_record, write_csv_lf
...
def args():
    ...
    p.add_argument("--deviation-proxy-met", default="true", choices=("true","false"))
    ...
    p.add_argument("--plan", type=Path, default=Path(
        "data/protocol/day26_trajectory_collection_plan.csv"))
    p.add_argument("--records", type=Path, default=Path(
        "data/protocol/day26_trajectory_collection_records.csv"))
    return p.parse_args()
...
def main():
    ...
    record=build_record(
        ...
        deviation_proxy_met=parse_bool(a.deviation_proxy_met),
    ...
    print("===== DAY26 RECORD UPDATED =====")
    for key in ("plan_row_id","episode_id","attempt_index","technical_valid",
                "experimental_valid","deviation_proxy_met","selected_canonical"):
        print(f"{key:20s}=",record[key])
    print("exclusion_reason     =",record["exclusion_reason"] or "<none>")
    if record["selected_canonical"]!="true":
        print("RESULT: registered as non-canonical attempt. Recollect SAME plan row with SAME frozen 40-60 mm forward deviation.")
        return 2
