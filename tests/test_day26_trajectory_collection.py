...
from evidencemm.day26_trajectory_collection import (
    TRAJECTORY_REPEAT_GROUPS,
    analyze_collection,
    build_record,
    canonical_records,
    evaluate_experimental_validity,
    expected_day26_rows_from_day22,
    selected_day24_clean_anchors,
    validate_day26_plan_shape,
    validate_final_analysis,
)

def _day22_rows():
    rows=[]
    trajectory=set(TRAJECTORY_REPEAT_GROUPS)
    target={"rcv2_g01","rcv2_g04","rcv2_g07","rcv2_g10","rcv2_g13"}
    gripper={"rcv2_g02","rcv2_g05","rcv2_g08","rcv2_g11","rcv2_g14"}
    for i in range(1,16):
        g=f"rcv2_g{i:02d}"
        rows += [
            ...
            {"plan_row_id":f"{g}_s04","pair_group_id":g,"slot_index":"4","slot_role":"controlled_cause","planned_physical_cause":"trajectory_execution_deviation","planned_intervention_type":"manual_bounded_trajectory_deviation","repeat_slot":"False"},
        ]
        if g in target:
...
def test_derive_plan_20():
    rows=expected_day26_rows_from_day22(_day22_rows())
    assert len(rows)==20
    validate_day26_plan_shape(rows)

def test_repeat_groups_exact():
    rows=expected_day26_rows_from_day22(_day22_rows())
    assert {r["pair_group_id"] for r in rows if r["repeat_slot"]=="true"} == set(TRAJECTORY_REPEAT_GROUPS)

def test_valid_frozen_failure():
    row=expected_day26_rows_from_day22(_day22_rows())[0]
    ok,reason=evaluate_experimental_validity(
        ...
        changed_factor_observable=True,deviation_proxy_met=True,
        safety_abort=False,hardware_fault=False)
    assert ok and reason==""

@pytest.mark.parametrize("field,reason",[
    ("task_success","trajectory_intervention_did_not_induce_failure"),
    ("intervention_applied","declared_trajectory_intervention_not_applied"),
    ("single_primary_intervention","multiple_primary_interventions"),
    ("changed_factor_observable","changed_factor_not_observable"),
    ("deviation_proxy_met","trajectory_deviation_proxy_not_met"),
])
...
def test_build_record_freezes_range():
    ...
    rec=build_record(plan_row=row,audit=_audit(),attempt_index=1,task_success=False,
        intervention_applied=True,single_primary_intervention=True,
        changed_factor_observable=True,deviation_proxy_met=True,safety_abort=False,
        hardware_fault=False,selected_canonical=True,operator_notes="")
    assert rec["selected_canonical"]=="true"
    assert rec["parameter_direction"]=="follower_forward"
    assert rec["parameter_min"]=="40" and rec["parameter_max"]=="60"
    assert rec["operational_measurement"]=="marked_lateral_waypoint_offset"
...
def test_duplicate_canonical_rejected():
    plan=expected_day26_rows_from_day22(_day22_rows())
...
def test_complete_analysis_without_raw_root():
    ...
    assert a["new_trajectory_canonical_count"]==20
    assert a["trajectory_failure_count"]==20
...
def test_empty_collection_in_progress():
    ...
    assert a["new_trajectory_canonical_count"]==0
    assert a["trajectory_failure_count"]==0
    assert a["clean_anchor_count"]==15
    assert a["complete_pair_group_count"]==0
    assert a["new_clean_collection_required"] is False
